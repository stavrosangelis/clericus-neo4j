...
class MenuItem {
  constructor({_id=null,menuId=null,label=null,order=0,parentId=0,type=null,objectId=null,link=null,target=null,accessGroup=[],status='private',createdBy=null,createdAt=null,updatedBy=null,updatedAt=null}) {
    this._id = null;
    if (_id!==null) {
      this._id = _id;
    }
  ...
  async save(userId) {
    ...
    if (!validateMenuItem.status) {
      ...
      if (typeof this._id==="undefined" || this._id===null) {
        ...
        query = "MATCH (n:MenuItem) WHERE id(n)="+this._id+" SET n="+nodeProperties+" RETURN n";
      }
  ...
  async countChildren() {
    ...
    let session = driver.session();
    let query = "MATCH (n:MenuItem) WHERE n.parentId="+this._id+" RETURN count(*) AS c";
