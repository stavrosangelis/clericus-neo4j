...
const getGraphData = async (req, resp) => {
  ...
  if (eventsLoad === 'true') {
    // nodes
    let eventsQuery =
      "MATCH (n:Event), (n)-[r]-() WHERE n.status='public' RETURN n, count(r) as count";
    let eventsCount = await countNodes(
      "MATCH (n:Event) WHERE n.status='public' RETURN count(n) as count"
    );
    ...
    // relations
    let eventsRQuery =
      "MATCH (n:Event)-[r]->(o) WHERE n.status='public' AND o.status='public' RETURN r";
    let eventsRCount = await countNodes(
      "MATCH (n:Event)-[r]->(o) WHERE n.status='public' AND o.status='public' RETURN count(r) as count"
    );
  ...
  if (organisationsLoad === 'true') {
    ...
    let organisationsQuery =
      "MATCH (n:Organisation), (n)-[r]-() WHERE n.status='public' RETURN n, count(r) as count";
    let organisationsCount = await countNodes(
      "MATCH (n:Organisation) WHERE n.status='public' RETURN count(n) as count"
    );
    let organisationsPromise = await loadBatch(
      organisationsQuery,
      organisationsCount
    );
    ...
    // relations
    let organisationsRQuery =
      "MATCH (n:Organisation)-[r]->(o) WHERE n.status='public' AND o.status='public' RETURN r";
    let organisationsRCount = await countNodes(
      "MATCH (n:Organisation)-[r]->(o) WHERE n.status='public' AND o.status='public' RETURN count(r) as count"
    );
    organisationsRelations = await loadBatch(
      organisationsRQuery,
      organisationsRCount
    );
    organisationsRelations = helpers.normalizeRelationsOutput(
  ...
  if (peopleLoad === 'true') {
    // nodes
    let peopleQuery =
      "MATCH (n:Person), (n)-[r]-() WHERE n.status='public' RETURN n, count(r) as count";
    let peopleCount = await countNodes(
      "MATCH (n:Person) WHERE n.status='public' RETURN count(n) as count"
    );
    ...
    // relations
    let peopleRQuery =
      "MATCH (n:Person)-[r]->(o) WHERE n.status='public' AND o.status='public' RETURN r";
    let peopleRCount = await countNodes(
      "MATCH (n:Person)-[r]->(o) WHERE n.status='public' AND o.status='public' RETURN count(r) as count"
    );
  ...
  if (resourcesLoad === 'true') {
    // nodes
    let resourcesQuery =
      "MATCH (n:Resource), (n)-[r]-() WHERE n.status='public' RETURN n, count(r) as count";
    let resourcesCount = await countNodes(
      "MATCH (n:Resource) WHERE n.status='public' RETURN count(n) as count"
    );
    ...
    // relations
    let resourcesRQuery =
      "MATCH (n:Resource)-[r]->(o) WHERE n.status='public' AND o.status='public' RETURN r";
    let resourcesRCount = await countNodes(
      "MATCH (n:Resource)-[r]->(o) WHERE n.status='public' AND o.status='public' RETURN count(r) as count"
    );
...
const getRelatedNodes = async (req, resp) => {
  ...
  let query =
    ...
    ']->(rn) WHERE id(n)=' +
    _id +
    " AND n.status='public' AND rn.status='public' AND NOT id(rn)=" +
    _id +
    ' RETURN distinct rn ORDER BY rn.label';
