...
const parseClassPiece = async (req, resp) => {
  ...
  fs.writeFile(
    jsonDir + fileName + '-faces.json',
    JSON.stringify(identifiedFaces),
    'utf8',
    (error) => {
  ...
  await fs.writeFile(
    jsonDir + fileName + '-text-status.json',
...
const getTextAnalysis = async (imagesDir, jsonDir, fileName) => {
  ...
  if (jsonData.completed === false) {
    ...
    if (
      ...
      await fs.writeFile(
        jsonDir + fileName + '-text.json',
        JSON.stringify(textData.data.recognitionResult),
        'utf8',
        (error) => {
          if (error) throw error;
...
const imageIptc = async (req, resp) => {
  let srcPath =
    '/Applications/XAMPP/htdocs/ahi/clericus/resources/images/processed/1971.jpg';
...
var highlightText = async (inputFile, text, outputFile, Canvas) => {
  if (
    typeof text.recognitionResult === 'undefined' ||
    typeof text.recognitionResult.lines === 'undefined'
  ) {
    return false;
  }
  ...
  for (let f in files) {
    ...
    for (let line in file) {
      ...
      if (Number(line) > 0) {
        ...
        // 1. insert person
        let match = await existingPerson(
          nameCol,
          dioceseText,
          ordinationOutput
        );
        ...
        await references.updateReference(dioceseReference);

        // 3.1. insert organisation/religious order
        if (nameCol.religiousOrder !== null) {
          let religiousOrder = await addReligiousOrder(
            nameCol.religiousOrder,
            userId
          );
          ...
          await references.updateReference(roReference);
        }

        // 4.1 insert matriculation date
        let matriculationDate = null;
        if (typeof matriculationOutput.date !== 'undefined') {
          matriculationDate = await addDate(
            matriculationOutput.date,
            null,
            userId
          );
        }
        ...
        if (
          typeof matriculationOutput.role !== 'undefined' &&
          matriculationOutput.role !== ''
        ) {
          matriculationRole = await addTaxonomyRole(
            matriculationOutput.role,
            matriculationClass._id,
            userId
          );
        }
        ...
        if (matriculationDate !== null) {
          ...
          let matriculationEvent = await addNewEvent(
            matriculationLabel,
            eventTypeMatriculation._id,
            userId
          );
        ...
        for (let j = 0; j < ordinationOutput.length; j++) {
          ...
          let ordinationEvent = await updateEvent(
            label,
            eventTypeOrdination._id,
            ordinationDateId,
            userId
          );
          ...
          if (role !== '') {
            let ordinationRole = await addTaxonomyRole(
              role,
              ordinationRanks._id,
              userId
            );
        ...
        if (typeof person._id !== 'undefined') {
          ...
          await references.updateReference(hamellReference);
        }
      }
    }
  }
  ...
  for (let line in file) {
    ...
    if (Number(line) > 0) {
      ...
      if (
        ...
        matriculationRole = await addTaxonomyRole(
          matriculationOutput.role.label,
          matriculationClass._id,
          userId
        );
      }
...
const existingPerson = async (name, diocese, ordination) => {
  ...
  if (nodesPromise.length > 0) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      let classpieces = await helpers.loadRelations(
        node._id,
        'Person',
        'Resource',
        false,
      ...
      let findClasspiece = classpieces.find((c) => {
        ...
        let next2 = num + 2;
        if (
          ordinationYears.indexOf(num) > -1 ||
          ordinationYears.indexOf(prev) > -1 ||
          ordinationYears.indexOf(prev2) > -1 ||
          ordinationYears.indexOf(next) > -1 ||
          ordinationYears.indexOf(next2) > -1
        ) {
          return true;
        }
        return false;
      ...
      if (
        typeof findDiocese !== 'undefined' &&
        typeof findClasspiece !== 'undefined'
      ) {
        match = true;
        person = node;
      }
...
const addPerson = async (nameCol, userId) => {
  ...
  let fnameSoundex = '';
  let lnameSoundex = '';
  let alternateAppelation = {};
  let alternateAppelationVal = [];
  ...
  if (
    typeof nameCol.alFirstName !== 'undefined' &&
    nameCol.alFirstName !== ''
  ) {
...
const afterIngestion = async (req, resp) => {
  ...
  const updateTerm = async (srcTerm, alternateTerm) => {
    ...
    for (let i = 0; i < triples.length; i++) {
      ...
      let newLabel = triple.target.label.replace(
        alternateTerm.label,
        srcTerm.label
      );
      let query3 = `MATCH (e:Event) WHERE id(e)=${Number(
        triple.target._id
      )} SET e.label="${newLabel}" return e`;
      await session
        .writeTransaction((tx) => tx.run(query3, {}))
        .catch((error) => {
          console.log(error);
        });
    }
    ...
    let query4 = `MATCH (t:TaxonomyTerm) WHERE id(t)=${Number(
      alternateTerm._id
    )}
    OPTIONAL MATCH (t)-[r]-(n)
    DELETE r,t`;
    await session
      .writeTransaction((tx) => tx.run(query4, {}))
      .catch((error) => {
        console.log(error);
      });
  ...
  let triples = [];
  for (let key in matriculationClasses) {
    ...
    for (let tkey in matriculationClass.alternateTerms) {
      ...
      let updatedTerm = await updateTerm(
        matriculationClass.term,
        matriculationClass.alternateTerms[tkey]
      );
