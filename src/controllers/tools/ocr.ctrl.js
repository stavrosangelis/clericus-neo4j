...
const ocrDocumentsDir = async (req, resp) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    let cronJob = schedule.scheduleJob(
      { start: startTime, rule: '15 * * * * *' },
      async () => {
        let textResults = await ocrTextQuery(
          directory,
          i,
          file,
          fileName,
    ...
    await new Promise((resolve, reject) => {
      fs.writeFile(targetFile, JSON.stringify(ocrText), 'utf8', (error) => {
        if (error) {
          ...
          results = {
            status: true,
            msg: `Text from file "${file}" saved successfully!`,
          };
          resolve(true);
        }
      });
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    for (let k = 0; k < regions.length; k++) {
      ...
      for (let m = 0; m < r.lines.length; m++) {
        ...
        if (x > c0x && w < c0w && x + w < c1x) c0.push(output);
        if (x > c1x && w < c1w && x + w < c2x) c1.push(output);
        if (x > c2x && w < c2w && x + w < c3x) c2.push(output);
        if (x > c3x && w < c3w && x + w < c4x) c3.push(output);
        if (x > c4x && w < c4w && x + w < c5x) c4.push(output);
        if (x > c5x && w < c5w && x + w < c6x) c5.push(output);
        if (x > c6x && w < c6w) c6.push(output);
      }
    }
    // sort by y
    c0.sort((a, b) => {
    ...
    await new Promise((resolve, reject) => {
      fs.writeFile(csvPath, csv, 'utf8', function (err) {
        if (err) {
          ...
          output.push(
            `CSV from file "${file}" created successfully at the path "${csvPath}"`
          );
          resolve(true);
        }
      });
...
const exportOrganisations = async (req, resp) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    let csv = await new Promise((resolve) => {
      let results = [];
  ...
  organisations.sort();
  let csvText =
    'Organisation,Organisation type, Cathedral city,\n' +
    organisations.join(',\n');
...
const correctOrganisations = async (req, resp) => {
  ...
  const organisationsFileHeaders = [
    'Organisation',
    'Organisation type',
    'Cathedral city',
    'Corrected organisation label',
  ];
  const organisationsFile = await new Promise((resolve) => {
    let results = [];
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    for (let line in csv) {
      ...
      if (
        organisation0 !== '' &&
        organisationsToBeReplaced.indexOf(organisation0) > -1
      ) {
        let replacement = organisations.find(
          (r) => r.current === organisation0
        );
      ...
      if (
        organisation1 !== '' &&
        organisationsToBeReplaced.indexOf(organisation1) > -1
      ) {
        let replacement = organisations.find(
          (r) => r.current === organisation1
        );
        replacements.push({
          file: file,
          line: line,
    ...
    let csvText =
      'First name,Middle name,Last name,Native of,App.,Title,Appointment,Died,Location of death,\n';
...
const processItems = async (req, resp) => {
  ...
  var organisations = await new Promise((resolve) => {
    let results = [];
    fs.createReadStream(`${directory}organisations.csv`)
      .pipe(csvParser(organisationsFileHeaders))
      .on('data', (data) => results.push(data))
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    for (let line in csv) {
      ...
      if (title !== '') {
        if (title.includes(',')) {
          ...
          for (let i = 0; i < titles.length; i++) {
            ...
            let rt = titlePairs.find(
              (t) => t[0].toLowerCase() === newTitle.toLowerCase()
            );
            if (i > 0) newTitleText += '; ';
            newTitleText += rt[1];
          }
          ...
          let rt = titlePairs.find(
            (t) => t[0].toLowerCase() === title.toLowerCase()
          );
          replaceTitle = rt[1];
        }
      }
      ...
      let deathEvent = '';
      if (row['Died'] !== '') {
        deathEvent = `${row['First name']} ${row['Middle name']} ${
          row['Last name']
        } died on ${row['Died']} [${normalizeDate(row['Died'])}]`;
      }
    ...
    let csvText =
      'First name,Middle name,Last name,Native of,Native of type,Native of Cathedral city,Native  Diocese,App.,Title,Appointment,Appointment type,Appointment Cathedral city,Appointment Diocese,Died,Location of death,Native event,Appointment event,Death event\n';
    for (let i = 0; i < newCsv.length; i++) {
      let row = newCsv[i];
...
const compareToDBProcessedItems = async (req, resp) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    let csvText =
      '#,ID,First name,Middle name,Last name,Native of,Native of type,Native of Cathedral city,Native  Diocese,App.,Title,Appointment,Appointment type,Appointment Cathedral city,Appointment Diocese,Died,Location of death,Native event,Appointment event,Death event\n';
    for (let i = 0; i < newCsv.length; i++) {
      let row = newCsv[i];
      ...
      for (let n = 0; n < nodes.length; n++) {
        ...
        let nodeDiocesePromise = await session
          .writeTransaction((tx) => tx.run(dnQuery, {}))
          .then((result) => {
            return result.records;
          })
          .catch((error) => {
        ...
        let nodesDiocese = helpers.normalizeRecordsOutput(
          nodeDiocesePromise,
          'rn'
        );
        let matchDiocese = nodesDiocese.find(
          (nd) =>
            nd.label.toLowerCase().trim() ===
            row['Native Diocese'].toLowerCase().trim()
        );
...
const ingestProcessedItems = async (req, resp) => {
  ...
  var organisationsList = await new Promise((resolve) => {
    let results = [];
    fs.createReadStream(`${directory}organisations.csv`)
      .pipe(csvParser(organisationsFileHeaders))
      .on('data', (data) => results.push(data))
  ...
  const organisationTypesTaxonomy = new Taxonomy({
  ...
  for (let key in organisations) {
    ...
    if (org.label !== '') {
      let newOrganisation = await insertOrganisation(
        org,
        userId,
        organisationTypesTaxonomy
      );
  ...
  let resourceMeath = await session
    .writeTransaction((tx) =>
      tx.run(
        'MATCH (n:Resource) WHERE n.label=~"History of the Diocese of Meath.*" RETURN n',
        {}
      )
    )
    .then((result) => {
  ...
  const headers = [
    ...
    'Native of',
    'Native of type',
    'Native of Cathedral city',
    'Native  Diocese',
    'App.',
    'Title',
    'Appointment',
    'Appointment type',
    'Appointment Cathedral city',
    'Appointment Diocese',
    'Died',
    'DB ID',
  ];
  for (let i = 0; i < files.length; i++) {
    ...
    for (let line in csv) {
      ...
      await updateReference(meathRef);

      // 3. link person to nativeOf organisation
      if (row['Native of'].trim() !== '') {
        let nativeOfOrg = await session
          .writeTransaction((tx) =>
            tx.run(
              `MATCH (n:Organisation) WHERE n.label="${row[
                'Native of'
              ].trim()}" return n`,
              {}
            )
        ...
        if (affiliationTypes.indexOf(row['Native of type']) > -1) {
          ...
          await updateReference(nativeAffiliationRef);
        }
        // native of
        if (nativeOfTypes.indexOf(row['Native of type']) > -1) {
          ...
          await updateReference(nativeRef);
        }
      }
      ...
      if (appointment !== '') {
        let appointmentOrg = await session
          .writeTransaction((tx) =>
            tx.run(
              `MATCH (n:Organisation) WHERE n.label="${appointment}" return n`,
              {}
            )
          )
          .then((result) => {
        ...
        if (
          appointmentType === 'School' &&
          seminaries.indexOf(appTitle) === -1
        ) {
        ...
        // 1. add event
        // 1.1 save event
        let eventType = await session
          .writeTransaction((tx) =>
            tx.run(
              `MATCH (n:TaxonomyTerm) WHERE n.labelId="${helpers.normalizeLabelId(
                eventTypeLabel
              )}" return n`,
              {}
            )
        ...
        await updateReference(eventOrganisationRef);

        // 1.4 relate event to time
        if (appointmentDate !== '') {
          let eventDate = await session
            .writeTransaction((tx) =>
              tx.run(
                `MATCH (n:Temporal) WHERE n.label="${appointmentDate}" return n`,
                {}
              )
            )
            .then((result) => {
          ...
          await updateReference(eventTemporalRef);
        }
          ...
          if (typeof appointmentOrgSpatial !== 'undefined') {
            let eventSpatialRef = {
              items: [
                { _id: event._id, type: 'Event', role: '' },
                {
                  _id: appointmentOrgSpatial.ref._id,
                  type: 'Spatial',
                  role: '',
                },
              ],
            ...
            await updateReference(eventSpatialRef);
          }
        }
        ...
        if (seminaries.indexOf(appTitle) > -1) {
          ...
          await updateReference(personSeminaryRef);
        }
      }
      if (appointmentDiocese !== '') {
        let appointmentDioceseOrg = await session
          .writeTransaction((tx) =>
            tx.run(
              `MATCH (n:Organisation) WHERE n.label="${appointmentDiocese}" return n`,
              {}
            )
          )
          .then((result) => {
        ...
        if (appointmentDioceseOrg == null) {
          ...
          let newAppointmentDioceseOrg = new Organisation(
            appointmentDioceseData
          );
          let newAppointmentDioceseOrgSave = await newAppointmentDioceseOrg.save(
            userId
          );
        ...
        await updateReference(appointmentDioceseAffiliationRef);
      }
      if (row['Died'] !== '') {
        let deathEventType = await session
          .writeTransaction((tx) =>
            tx.run(
              `MATCH (n:TaxonomyTerm) WHERE n.labelId="${helpers.normalizeLabelId(
                'Death'
        ...
        await updateReference(deceasedEventPersonRef);

        // relate deceased event date
        let deceasedEventDate = await session
          .writeTransaction((tx) =>
            tx.run(
              `MATCH (n:Temporal) WHERE n.label="${deathDate}" return n`,
              {}
            )
          )
          .then((result) => {
        ...
        await updateReference(deceasedEventTemporalRef);
      }
    }
  }
  session.close();
...
const insertOrganisation = async (
  ...
  await insertOrganisationType(
    organisation.type,
    userId,
    organisationTypesTaxonomy._id
  );
...
const insertOrganisationType = async (
  organisationType,
  userId,
  organisationTypesTaxonomyId
) => {
  ...
  if (
    typeof newOrganisationType._id === 'undefined' ||
    newOrganisationType._id === null
  ) {
...
const fixDBIDS = async (req, resp) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    let dbCsv = await new Promise((resolve) => {
      let results = [];
