...
class Usergroup {
  ...
  async save(userId) {
    ...
    if (!validateUsergroup.status) {
      ...
      if (this.isDefault) {
        if (this.isAdmin) {
          let output = {
            error: ['You can only set a public usergroup as default'],
            status: false,
            data: [],
          };
          return output;
        ...
        let queryDefault =
          'MATCH (n:Usergroup) WHERE n.isDefault=true SET n.isDefault=false RETURN n';
        await session
          .run(queryDefault, {})
          .then((result) => {
            return result;
  ...
  async delete() {
    ...
    if (this.isDefault) {
      let output = {
        error:
          'This is the default user group. If you wish to remove it please set another group as default first.',
        status: false,
        data: [],
      };
      return output;
    ...
    let count = await session
      .writeTransaction((tx) =>
        tx.run(
          'MATCH (n:User)-[r]->(rn:Usergroup) WHERE id(rn)=' +
            this._id +
            ' RETURN count(*) AS c'
        )
      )
    ...
    if (count > 0) {
      let output = {
        error:
          'This usergroup is associated with users. If you wish to delete it you must first remove these associations.',
        status: false,
        data: [],
      };
      return output;
...
const getUsergroupsQuery = async (query, limit) => {
  ...
  let count = await session
    .writeTransaction((tx) => tx.run('MATCH (n:Usergroup) RETURN count(*)'))
    .then((result) => {
      session.close();
      let resultRecord = result.records[0];
      let countObj = resultRecord.toObject();
