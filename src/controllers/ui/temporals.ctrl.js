...
const getTemporals = async (req, resp) => {
  ...
  if (!params.returnResults) {
    ...
    let data = await getTemporalsQuery(query, params.match, params.queryParams, params.limit);
    if (data.error) {
      resp.json({
        status: false,
        data: [],
...
const getTemporalsPrepareQueryParams = async(req)=>{
  ...
  queryOrder = ` ORDER BY date(datetime({epochmillis: apoc.date.parse(n.startDate,"ms","dd-MM-yyyy")})) `;
...
const getTemporal = async(req, resp) => {
  ...
  if (typeof temporal!=="undefined") {
    ...
    let people = await helpers.loadRelations(_id, "Temporal", "Person", false, null, "rn.lastName");
    let resources = await helpers.loadRelations(_id, "Temporal", "Resource", false);
