  ...
  (function () {
    function T(a) {
      ...
      function v(e) {
        ...
        for (g = 1; g < d.length; ++g) d[g] === -1 && (d[g] = ++A);
        for (h = g = 0; g < c; ++g)
          (k = a[g]),
            k === '('
              ? (++h, d[h] || (a[g] = '(?:'))
    ...
    function V(a) {
      for (var d = void 0, f = a.firstChild; f; f = f.nextSibling)
        var b = f.nodeType,
          d =
            b === 1 ? (d ? a : f) : b === 3 ? (W.test(f.nodeValue) ? a : d) : d;
      return d === a ? void 0 : d;
    }
    function G(a, d) {
      function f(a) {
        for (
          ...
          if (typeof q === 'string') h = !1;
            ...
            !(h = q.length >= 5 && q.substring(0, 5) === 'lang-') ||
              (g && typeof g[1] === 'string') ||
              ((h = !1), (q = 'src'));
            h || (r[z] = q);
          }
      ...
      (function () {
        for (
          ...
          var w = f[c];
          var r = w[3];
          if (r) for (var e = r.length; --e >= 0; ) b[r.charAt(e)] = w;
          w = w[1];
          r = '' + w;
      ...
      try {
        ...
        try {
          for (; b < m; ) {
            ...
            var B = l[b + 1];
            var D;
            if (B.nodeType !== 1 && (D = A.substring(f, q))) {
              v && (D = D.replace(d, '\r'));
              B.nodeValue = D;
    ...
    var C = [C, 'case,done,elif,esac,eval,fi,function,in,local,set,then,until'];
    var S = /^(DIR|FILE|vector|(de|priority_)?queue|list|stack|(const_)?iterator|(multi)?(set|map)|bitset|u?(int|float)\d*)\b/;
    var W = /\S/;
    var X = y({
      keywords: [
        H,
    ...
    var Y = (E.PR = {
      ...
      prettyPrint: (E.prettyPrint = function (a, d) {
        function f() {
          for (
            ...
            for (var d = t[p], l = g, m = d; (m = m.previousSibling); ) {
              var n = m.nodeType;
              var u = (n === 7 || n === 8) && m.nodeValue;
              if (
                u
                  ? !/^\??prettify\b/.test(u)
                  : n !== 3 || /\S/.test(m.nodeValue)
              )
                break;
              if (u) {
                l = {};
          ...
          p < t.length ? E.setTimeout(f, 250) : typeof a === 'function' && a();
        }
        for (
        ...
        var p = 0;
        var w = /\blang(?:uage)?-([\w.]+)(?!\S)/;
        var r = /\bprettyprint\b/;
        var e = /\bprettyprinted\b/;
        var y = /pre|xmp/i;
        var z = /^code$/i;
        var q = /^(?:pre|code|xmp)$/i;
        var g = {};
        f();
      }),
    });
